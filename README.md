...
- **Favoriten:** Einzelne Vorträge können per Stern neben der Uhrzeit markiert werden; die IDs werden pro Veranstaltung im `localStorage` des Browsers gespeichert. Über die Toolbar kann die Ansicht auf gespeicherte Favoriten eingeschränkt werden (`?favorites=1`); angeboten werden ein Teilen-Link mit Hash-Fragment (`#f=1/…`), die Übernahme der Auswahl aus einem Teilen-Link als eigene Favoriten („Import“) sowie das Leeren aller Favoriten auf dem Gerät (mit Bestätigungsdialog). Stern und Toolbar werden beim Drucken nicht ausgegeben.
